...
export default function VetMind() {
  ...
  const [slackChannelsSearched, setSlackChannelsSearched] = useState(0);

  const slackFetch = async (url: string, init?: RequestInit) => {
  ...
  const handleSlackSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    const q = slackQuery.trim();
    if (q.length < 2) {
      setSlackError("Bitte Suchbegriff mit mindestens 2 Zeichen eingeben.");
      return;
    }
    setSlackError(null);
    setSlackLoading(true);
    try {
      const res = await slackFetch(`/api/slack/search?q=${encodeURIComponent(q)}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Suche fehlgeschlagen.");
      setSlackResults(data.results || []);
      setSlackTotal(data.total || 0);
      setSlackChannelsSearched(data.channelsSearched || 0);
    } catch (err) {
      setSlackError(err instanceof Error ? err.message : "Unbekannter Fehler");
      setSlackResults([]);
    } finally {
  ...
  const handleSlackInsert = (r: SlackSearchResult) => {
    const when = new Date(r.date).toLocaleString("de-DE", {
      day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit",
    });
    const header = `[Slack: #${r.channelName}${r.userName ? ` · ${r.userName}` : ""} · ${when}]`;
    ...
    <div style={{
      ...
      <div className="mt-3 bg-white border border-gray-200 rounded-2xl overflow-hidden">
        ...
        >
          ...
          <span className={`text-gray-400 transition-transform ${slackOpen ? "rotate-180" : ""}`}>▾</span>
        </button>

        {slackOpen && (
          <div className="border-t border-gray-100 p-4 space-y-3">
            <form onSubmit={handleSlackSearch} className="flex gap-2">
              <input
                type="text"
                value={slackQuery}
                onChange={(e) => { setSlackQuery(e.target.value); if (slackError) setSlackError(null); }}
                placeholder="🔍 In allen Channels suchen..."
                className="flex-1 px-3 py-2 rounded-lg border border-gray-200 text-sm outline-none focus:border-[#0f6b74] focus:ring-2 focus:ring-[#0f6b74]/15"
              />
              <button
                type="submit"
                disabled={slackLoading || slackQuery.trim().length < 2}
                className="px-4 py-2 rounded-lg text-xs font-semibold text-white bg-[#0f6b74] hover:bg-[#0d5c64] disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
              >
                {slackLoading ? "Sucht..." : "Suchen"}
              </button>
            </form>
            ...
            {!slackLoading && slackResults.length === 0 && !slackError && slackQuery.trim().length >= 2 && slackTotal === 0 && slackChannelsSearched > 0 && (
              <div className="text-xs text-gray-500">
                {`Keine Treffer für „${slackQuery}" in ${slackChannelsSearched} Channels.`}
              </div>
            )}

            {slackResults.length > 0 && (
              <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
                <div className="text-xs text-gray-500 sticky top-0 bg-white pb-1">
                  {slackTotal > slackResults.length
                    ? `${slackResults.length} von ${slackTotal} Treffern (${slackChannelsSearched} Channels)`
                    : `${slackResults.length} Treffer (${slackChannelsSearched} Channels)`}
                </div>
                {slackResults.map((r) => {
                  const when = new Date(r.date).toLocaleString("de-DE", {
                    day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit",
                  ...
                  return (
                    <div key={r.id} className="rounded-xl border border-gray-200 p-3">
                      <div className="flex items-baseline gap-2 flex-wrap text-xs text-gray-500">
                        <span className="font-semibold text-[#0f6b74]">#{r.channelName}</span>
                        {r.userName && <span>· {r.userName}</span>}
                        <span>· {when}</span>
                      </div>
                      <div className="text-sm text-gray-800 mt-1 whitespace-pre-wrap break-words line-clamp-4">
                        {r.text}
                      </div>
                      <div className="flex flex-wrap gap-2 mt-2">
                        <button
                          type="button"
                          onClick={() => handleSlackInsert(r)}
                          className="px-3 py-1.5 rounded-lg border border-gray-200 text-xs font-semibold text-gray-700 hover:border-[#0f6b74] hover:text-[#0f6b74] transition-colors"
                        >
                          📥 In Chat einfügen
                        </button>
